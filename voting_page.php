...
$readPos = new Voting();
$rtnReadPos = $readPos->READ_POSITION($camp);
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-md-offset-3">
            ...
            if(isset($_POST['vote'])) {
                ...
                $rtnInsertVote = $insertVote->VOTE_NOMINEE($camp, $pos, $candidate_id, $voters_id);
            }

            ?>
            <div class="voting-con">

                <h4 style="text-align: center;"><?php echo $camp; ?> Voting Page</h4><hr />
                <?php while($rowPos = $rtnReadPos->fetch_assoc()) { ?>
                <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" role="form">
                    <p class="help-block"><b><?php echo $rowPos['pos']; ?></b></p>
                        <?php
                        $readNominee = new Voting();
                        $rtnReadNominee = $readNominee->READ_NOMINEES($camp, $rowPos['pos']);
                        ?>
                    ...
                    $rtnValVote = $validateVote->VALIDATE_VOTE($camp, $rowPos['pos'], $_SESSION['ID'])
                    ?>
