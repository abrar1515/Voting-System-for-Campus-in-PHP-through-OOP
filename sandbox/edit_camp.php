        ...
        <div class="row">
            <div class="col-md-4 col-md-offset-4">
                ...
                if(isset($_POST['update'])) {
                    ...
                    $campus_id = trim($_POST['camp_id']);

                    $updateCamp = new Campus();
                    $rtnUpdateCamp = $updateCamp->UPDATE_CAMPUS($campus, $campus_id);
                }
                ?>
                <h4>Edit Campus</h4><hr>
                <?php
                if(isset($_GET['id'])) {
                    $camp_id = trim($_GET['id']);

                    $editCamp = new Campus();
                    $rtnEditCamp = $editCamp->EDIT_CAMPUS($camp_id);
                }
                ?>

                <?php if($rtnEditCamp) { ?>
                <form method="POST" action="<?php $_SERVER['PHP_SELF']; ?>" role="form">
                    <?php while($rowEditCamp = $rtnEditCamp->fetch_assoc()) { ?>
                    <div class="form-group-sm">
                        <label for="campus">Campus</label>
                        <input required type="text" name="campus" value="<?php echo $rowEditCamp['camp']; ?>" class="form-control">
                    </div><hr>
                    <div class="form-group-sm">
                        <input type="hidden" name="camp_id" value="<?php echo $rowEditCamp['id']; ?>">
                        <input type="submit" name="update" value="Update" class="btn btn-info">
                    </div>
                    <?php } //End while ?>
                </form>
                <?php $rtnEditCamp->free(); ?>
                <?php } //End if ?>
            </div>
        </div>
    </div>
